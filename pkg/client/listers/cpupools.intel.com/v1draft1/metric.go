...
type MetricLister interface {
	...
	List(selector labels.Selector) (ret []*v1draft1.Metric, err error)
...
// List lists all Metrics in the indexer.
func (s *metricLister) List(selector labels.Selector) (ret []*v1draft1.Metric, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1draft1.Metric))
	})
	return ret, err
}
...
type MetricNamespaceLister interface {
	...
	Get(name string) (*v1draft1.Metric, error)
...
// List lists all Metrics in the indexer for a given namespace.
func (s metricNamespaceLister) List(selector labels.Selector) (ret []*v1draft1.Metric, err error) {
...
// Get retrieves the Metric from the indexer for a given namespace and name.
func (s metricNamespaceLister) Get(name string) (*v1draft1.Metric, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1draft1.Resource("metric"), name)
	}
	return obj.(*v1draft1.Metric), nil
}
