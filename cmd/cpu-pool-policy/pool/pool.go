...
//
// The pool policy maintains a set of CPU pools which are used to allocate
// CPU to containers. Configuration for how to split up the available CPUs
// to pools is provided externally, typically as a cluster-wide ConfigMap.
// Containers request CPU from a particular pool by requesting an extended
// resource unique (within the node) to the pool. There is a number of pre-
// defined pools with special semantics:
//
//  - reserved:
//    The set of CPUs dedicated to system- and kube-reserved pods and other
//    processes. Pods in the kube-system namespace will get allocated CPU
//    from this pool.
//
//  - default:
//    Pods which do not request any particular pool are allocated CPUs from
//    this pool.
//
//  - offline:
//    CPUs assigned to this pool are taken offline, typically to disable
//    hyperthreading for its sibling core.
//
//  - ignored:
//    CPUs in this pool are ignored.
//
...
func (ps *PoolSet) AllocateCPUs(id string, pool string, numCPUs int) (cpuset.CPUSet, error) {
	...
	ps.updatePoolMetrics(pool, true)
...
func (ps *PoolSet) ReleaseCPU(id string) {
	...
	ps.updatePoolMetrics(c.pool, true)
	ps.reconcileConfig()
}
...
func (ps *PoolSet) updatePoolMetrics(pool string, publish bool) {
	if name, s, e, c, u := ps.getPoolMetrics(pool); name != "" {
		if err := ps.metrics.UpdatePool(name, s, e, c, u); err != nil {
